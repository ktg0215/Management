...
async function runTests() {
  console.log('🔍 AppLayout修正後の全ページテスト開始\n');
  ...
  results.forEach((result, index) => {
    ...
    if (result.status === 200) {
      ...
      console.log(`   ✓ Title Tag: ${result.hasTitle ? 'Yes' : 'No'}`);
      console.log(`   ✓ Scripts: ${result.hasScripts ? 'Yes' : 'No'}`);
      console.log(`   ✓ Stylesheets: ${result.hasStyles ? 'Yes' : 'No'}`);

      if (result.hasError404 || result.hasError500) {
        console.log(`   ⚠️  Warning: Error indicators found in content`);
      }
    } else {
      failureCount++;
  ...
  const fs = require('fs');
  fs.writeFileSync(
    'C:/job/project/page-test-results.json',
    JSON.stringify({ timestamp: new Date().toISOString(), results, summary: { total: pages.length, success: successCount, failure: failureCount }}, null, 2)
  );
  console.log('\n💾 詳細結果を保存しました: C:/job/project/page-test-results.json');
}
