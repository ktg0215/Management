...
import { Field } from '@/types/monthly-sales';

interface MonthlySalesCsvExportModalProps {
  isOpen: boolean;
  onClose: () => void;
  onExport: (options: {
    startYear: number;
...
export const MonthlySalesCsvExportModal: React.FC<MonthlySalesCsvExportModalProps> = ({
  isOpen,
  onClose,
  onExport,
  availableFields,
  ...
  // 初期選択：すべてのフィールドを選択
  useEffect(() => {
    if (isOpen && sortedFields.length > 0) {
      setSelectedFields(sortedFields.map(f => f.name));
    }
  }, [isOpen, sortedFields]);
  ...
  const toggleAllFields = () => {
    if (selectedFields.length === sortedFields.length) {
      setSelectedFields([]);
    } else {
      setSelectedFields(sortedFields.map(f => f.name));
    }
        ...
        <div className="flex-1 overflow-y-auto p-6">
          ...
          <div>
            <div className="flex items-center justify-between mb-4">
              ...
              >
                {selectedFields.length === sortedFields.length ? 'すべて解除' : 'すべて選択'}
              </button>
            </div>
