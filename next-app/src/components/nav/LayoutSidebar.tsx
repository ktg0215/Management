...
import { User, LogOut, Clock, CalendarCheck, BarChart3, TrendingUp, PieChart, CreditCard, Building, Users, UserPlus, Building2, FileText, Menu, ChevronLeft } from "lucide-react";
...
export const LayoutSidebar: React.FC = () => {
  ...
  // Always use the same value for server and client to prevent hydration mismatch
  const [actualCollapsed, setActualCollapsed] = useState(false);

  useEffect(() => {
    setIsClient(true);
    // Apply actual collapsed state after hydration
    if (isHydrated) {
      setActualCollapsed(isCollapsed);
    }
    ...
    >
      ...
      <div className="flex-1 overflow-y-auto bg-white">
          ...
          <div className="px-4 py-4">
            <div className="flex flex-col items-start p-3 bg-blue-50 rounded-lg">
              ...
              <div className="flex items-center">
                ...
                <span className="ml-2 font-medium">{isClient ? (user?.nickname || 'ユーザー') : 'ユーザー'}</span>
              </div>
            </div>
          </div>
        )}
        <div className="px-2 space-y-1 py-4">
          ...
          <NavItem to="/admin/dashboard" icon={<BarChart3 />} label="売上管理" isCollapsed={actualCollapsed} />
          ...
          <NavItem to="/admin/pl-create" icon={<FileText />} label="損益計算書作成" isCollapsed={actualCollapsed} />
